...
async def _handle_auth_error(bot: aiogram.Bot, user_id: int) -> bool:
    """Инкремент счётчика 401/403/422. Если >=3 подряд — кик. Возвращает True если кикнули."""
...
async def check_homework_start(bot: aiogram.Bot):
    ...
    # ponytail: jitter размазывает нагрузку — не все инстансы/циклы бьют в msapi в :59:00
    ...
    # ponytail: батчи 20 + sleep 2с, <100 юзеров пролетит за секунды, при 1000 не убьёт msapi
    batch_size = 20
