...
from ....storage import MARKS_DOMAIN, MARKS_ENDPOINT
...
app.mount("/{MARKS_ENDPOINT}", StaticFiles(directory=UPLOAD_DIR), name=MARKS_ENDPOINT)
...
async def remove_file_after_delay(path: str, delay: int):
    ...
    if os.path.exists(path):
        os.remove(path)
        logger.info(f"Файл {path} успешно выпилен.")
...
async def upload_file(
    ...
    return f"https://{MARKS_DOMAIN}/{MARKS_ENDPOINT}/{unique_filename}"